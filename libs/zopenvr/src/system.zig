...
pub fn getD3D9AdapterIndex(self: Self) ?i32 {
    const adapter_index: i32 = self.function_table.GetD3D9AdapterIndex();
    if (adapter_index == -1) {
        return null;
    } else {
...
pub fn getEventTypeNameFromEnum(self: Self, event_type: EventType) []const u8 {
